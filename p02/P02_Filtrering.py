...
filenameMeas = "P02_Filtrering_Data.txt"
...
filenameCalcOnline = "P02_Filtrering_Data_Kalkulert.txt"
# --------------------------------------------------------------------
        ...
        while True:
            ...
            if len(filenameCalcOnline)>4:
                if k == 0:
                    ...
                    CalculationsToFileHeader += "0=Temp_FIR, 1=Temp_IIR, \n"
                    CalculationsToFileHeader += "2= \n"
                    robot["calculations"].write(CalculationsToFileHeader)
                CalculationsToFile = ""
                CalculationsToFile += str(Temp_FIR[-1]) + "\n"
                #CalculationsToFile += str(Temp_IIR[-1]) + "\n"
...
def MathCalculations(Lys, Temp_FIR):
    m = 100
    k = len(Lys)
    Temp_FIR.append((1/m)* sum(Lys[:-1]))
